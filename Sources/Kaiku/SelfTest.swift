...
/// `Kaiku --selftest-audio [seconds]`: records mic + system audio for a few seconds
...
enum SelfTest {
    static func runAudio(seconds: Double) -> Int32 {
        ...
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("kaiku-selftest-\(Int(Date().timeIntervalSince1970))")
    ...
    /// `Kaiku --selftest-recovery`: a child process writes a CAF track the same way a
    ...
    static func runRecovery() -> Int32 {
        setvbuf(stdout, nil, _IONBF, 0)
        AppSettings.registerDefaults()
        let base = FileManager.default.temporaryDirectory.appendingPathComponent("kaiku-recovery-\(Int(Date().timeIntervalSince1970))")
    ...
    /// `Kaiku --selftest-devicechange`: checks that tracks stay aligned when a source changes.
    ...
    static func runDeviceChange() -> Int32 {
        setvbuf(stdout, nil, _IONBF, 0)
        AppSettings.registerDefaults()
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("kaiku-devices-\(Int(Date().timeIntervalSince1970))")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    /// `Kaiku --selftest-mute [--force]`: mutes every microphone, checks it, simulates an
    ...
    /// `Kaiku --selftest-detect`: prints the processes using audio input right now and
    ...
    /// `Kaiku --selftest-trim`: builds 4 s tone, 10 s silence, 4 s tone, trims it the
    /// way transcription does and checks the time mapping. Offline, no devices used.
    static func runTrim() -> Int32 {
        setvbuf(stdout, nil, _IONBF, 0)
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("kaiku-trim-\(Int(Date().timeIntervalSince1970))")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    /// `Kaiku --selftest-audiotools`: checks the offline conversions used by the providers
    /// (mix, 16 kHz WAV, compression, chunking) on generated audio. No devices used.
    static func runAudioTools() -> Int32 {
        setvbuf(stdout, nil, _IONBF, 0)
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("kaiku-tools-\(Int(Date().timeIntervalSince1970))")
