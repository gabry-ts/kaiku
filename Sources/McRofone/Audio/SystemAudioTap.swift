...
final class SystemAudioTap {
    ...
    private let queue = DispatchQueue(label: "mcrofone.system-tap", qos: .userInitiated)
    /// Builds, tears down and handles device events, one at a time.
    private let control = DispatchQueue(label: "mcrofone.system-tap.control", qos: .userInitiated)
    ...
    private func build() throws {
        ...
        desc.uuid = UUID()
        desc.name = "mc.Rofone system audio"
        ...
        let aggregate: [String: Any] = [
            kAudioAggregateDeviceNameKey: "mc.Rofone Tap",
